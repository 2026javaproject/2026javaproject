...
| 주차(날짜) | 목표 | 주요 작업 | 산출물 |
|---|---|---|---|
| 1주차 (05/20~05/26) | 구조 확정 및 기본 골격 구현 | 패키지 구조 생성, `GameMain`/`GamePanel` 기본 틀, 화면 전환, 공통 상수 정리 | 실행 가능한 기본 프레임, 구조도 초안 |
| 2주차 (05/27~06/02) | 핵심 기능 구현 및 통합 | 플레이어 이동/발사, 적 스폰, 충돌 판정, 점수/스테이지, HUD, 보스/아이템 | 게임 플레이 가능한 버전(v1) |
...
- 구조의 명확성: 기능 요소와 구조를 매핑해 요구사항 반영을 명시한다.
- 역할 분담의 적절성: 역할 범위를 UI/로직/문서로 명확히 구분한다.
- 일정 계획의 현실성: **2026-05-20 ~ 2026-06-11 (약 3주)** 기준으로 구성한다.
- 문서 정리의 완성도: 표/다이어그램/이미지로 가독성을 확보한다.

## 6. 최종 제출용 요약

- 본 프로젝트는 `main → screen → entity/manager → util` 흐름으로 구성된다.
- 화면은 메뉴/게임/게임오버로 분리하고, 핵심 로직은 Manager 계층에서 통합 처리한다.
- 역할 분담은 임대훈(UI/문서/저장), 강민석(게임 로직/엔티티) 기준으로 나눈다.
- 일정은 2026-05-20 ~ 2026-06-11 기준으로 운영한다.
- 이미지와 도식은 README에 직접 삽입하여 발표/평가 시 바로 확인 가능하게 구성한다.
